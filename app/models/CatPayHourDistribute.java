...
@Entity(name = CatPayHourDistribute.TABLE_NAME)
@JsonIgnoreProperties(value = {
        "entityId", "tableHashKey", "persistent", "tableName", "idName", "idColumn", "id"
})
public class CatPayHourDistribute extends GenericModel implements
		PolicySQLGenerator {

	@Transient
	private static final Logger log = LoggerFactory
			.getLogger(CatPayHourDistribute.class);

	@Transient
	public static final String TABLE_NAME = "cat_payhour_distribute";

	@Transient
	public static CatPayHourDistribute EMPTY = new CatPayHourDistribute();
	...
	public CatPayHourDistribute(Long cid) {
		super();
		this.cid = cid;
	}
	
	public CatPayHourDistribute(Long cid, int clock0, int clock1, int clock2,
	...
	public static CatPayHourDistribute findByCid(Long cid) {
		if (cid == null) {
			return null;
		}
		...
		return new JDBCBuilder.JDBCExecutor<CatPayHourDistribute>(dp, query,
				cid) {
			@Override
			public CatPayHourDistribute doWithResultSet(ResultSet rs)
					throws SQLException {
				if (rs.next()) {
					return parseCatPayHourDistribute(rs);
				} else {
					return null;
				}
			}
	...
	private static CatPayHourDistribute parseCatPayHourDistribute(ResultSet rs) {
		try {
			return new CatPayHourDistribute(rs.getLong(1), rs.getInt(2), rs.getInt(3),
	...
	public static List<CatPayHourDistribute> findValidList(int offset, int limit) {
		String query = "select " + SelectAllProperties + " from " + TABLE_NAME
				+ " limit ?,? ";

		return new JDBCBuilder.JDBCExecutor<List<CatPayHourDistribute>>(dp, query, offset, limit) {
			@Override
			public List<CatPayHourDistribute> doWithResultSet(ResultSet rs)
					throws SQLException {
				List<CatPayHourDistribute> res = new ArrayList<CatPayHourDistribute>();
				while (rs.next()) {
					res.add(parseCatPayHourDistribute(rs));
				}
				return res;
			}
		}.call();
	...
	public static abstract class CatPayHourDistributeBatchOper implements Callable<Boolean> {
        public int offset = 0;
        ...
        public CatPayHourDistributeBatchOper(int offset, int limit) {
            this.offset = offset;
            this.limit = limit;
        }

        public CatPayHourDistributeBatchOper(int limit) {
            this.limit = limit;
        }

        public List<CatPayHourDistribute> findNext() {
            return CatPayHourDistribute.findValidList(offset, limit);
        }

        public abstract void doForEachDistribute(CatPayHourDistribute distribute);
        ...
        public Boolean call() {
            ...
            while (true) {
                ...
                List<CatPayHourDistribute> findList = findNext();
                if (CommonUtils.isEmpty(findList)) {
                    return Boolean.TRUE;
                }

                for (CatPayHourDistribute distribute : findList) {
                    offset++;
