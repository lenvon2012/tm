...
((function ($, window) {
    ...
    boyvon.allItems = $.extend({
        ...
        initUserInfo : function(){
            $.get("/paipaipromote/getUserInfo",function(data){
        ...
        initSearchArea : function(){
            /*$.get("/paipaiitems/sellerCatCount",function(data){
            ...
            $.get("/paipaiitems/itemCatCount",function(data){
        ...
        createItemsBody : function(data){
            ...
            itembody.find('.switch').click(function(){
                if($(this).hasClass("switchOn")) {
                    ...
                    dataParams.status = 5;
                        $.post("/paipaipromote/removePopularized", dataParams, function(){
                    ...
                    if(parseInt($.cookie("totalNum")) > parseInt($.cookie("popularizedNum"))) {
                        ...
                        $.post("/paipaipromote/addPopularized", dataParams, function(){
                            $.cookie("popularizedNum",parseInt($.cookie("popularizedNum")) + 1);
                            $('.popularized').text($.cookie("popularizedNum"));
                            $('.remain ').text($.cookie("totalNum")-$.cookie("popularizedNum"));
        ...
        createItem : function(oneitem){
            ...
            var url = "http://auction2.paipai.com/" + oneitem.numIid;
            item.append($('<a href="javascript:void(0)" class="boyvonunselected selectPic" numIid="'+oneitem.numIid+'"></a>'));
            ...
            if (boyvon.isTryLevel == true && oneitem.popularized) {
                var html = '' +
                    ...
                    '';
                var btnObj = $(html);
                btnObj.find(".look-popularized-btn").attr("href", "/tryshare?numIid=" + oneitem.numIid);
                item.append(btnObj);
            } else {
    ...
    boyvon.Event = $.extend({
        ...
        setAllAddBatch : function(){
            ...
            $('.batchAllAddPopularize').click(function(){
                ...
                $.get("/paipaipromote/addPopularizedAll", dataParams, function(data){
                    if(data.res){
                        TM.Alert.load("宝贝已添加到推广计划中~",400,200,function(){
                            location.reload();
                        });
...
//                } else {
                ...
                dataParams.status = 5;
                    $.get("/paipaipromote/removePopularizedAll", dataParams, function(data){
                        TM.Alert.load("宝贝已从推广计划中取消~",400,200,function(){
                            location.reload();
                        });
                    });
