...
//                                    "background-image:url(http://img02.taobaocdn.com/imgextra/i2/79742176/T2_yxjXYlaXXXXXXXX-79742176.gif)'></a></span></div>");
         ...
         clickShow:function(container){
              container.find(".templateWidth").unbind("click").click(function(){
                  ...
                  window.location.href="/Relation/relationOper?size=" + size + "&count=" + count;
                  $(".set_step").css('display','none');
        ...
        getParamData:function(){
            ...
            if(templateWidth == 750 || templateWidth == 790 || templateWidth == 950){
                ...
                window.location.href="/relation/relationOper?size=750&count=0";
            }
    ...
    associate.listItemsDiag = $.extend({
                ...
                ajax: {
                    ...
                    callback:function(data){
                        if(data.res.length > 0){
                            ...
                            $(".itemsList_Area").empty();
                            $(".itemsList_Area").append('<tr><td style="text-align: center;width:880px;color: #434a54;font-size: 24px;">抱歉，没有找到相关的宝贝<td><tr>');
                        }
        ...
        showDialog:function(planId,results){
            ...
            container.find(".to_add_item").unbind("click").click(function(){
                ...
                TM.Alert.load(html, 880,800, function(){TM.associate.insertItem.closeWindow(planId);},function(){return false},"点击图片选择宝贝，点击标题查看宝贝");
...
//                        option.html(data[i].name+"("+data[i].count+")");
                    ...
                    callback:function(data){
                        if(data != null){
                            if(data.res.length > 0){
                               ...
                               $(".toPutItemListArea").empty();
                               $(".toPutItemListArea").append('<tr><td style="text-align: center;color: #434a54;font-size: 24px;width:100%;">抱歉，没有找到相关的宝贝<td><tr>');
                            }
        ...
        toPutResultDialog:function(data,actStr){
            if(data != null){
               ...
               if(fail == 0){
                    TM.Alert.load('<div>' + actStr + '成功：<span class="successNum">'+ success +'</span>个</div>',300,250,function(){window.location.href="/relation/myRelation?type=1"},false,"投放结果");
               }else{
                   ...
                   TM.Alert.load(tableDiv,780,500,function(){window.location.href="/relation/myRelation?type=1"},false,"投放结果");
               }
            }
        },
