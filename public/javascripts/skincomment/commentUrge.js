var TM = TM || {};
((function ($, window) {
    TM.CommentUrge = TM.CommentUrge || {};

    var CommentUrge = TM.CommentUrge;

    CommentUrge.Init = CommentUrge.Init || {};
    CommentUrge.Init = $.extend({
        init : function(container){
            ...
            TM.CommentUrge.Util.initSearchParams();

            TM.CommentUrge.Util.initRemarkSet();

            // update the trade-search-table-tbody content
            TM.CommentUrge.TradeSearch.search();

            TM.CommentUrge.Event.setStaticEvent();

            TM.CommentUrge.Init.initShortMsg(container);

            TM.CommentUrge.Init.initMsgLogs();
        ...
        initShortMsg:function(container){
            ...
            shortMsgBtn.click(function(){
                ...
                TM.CommentUrge.Show.showSendMsgWindow(subCheck);
        ...
        initMsgLogs:function(){
            $("#msg-log-btn").click(function(){
                CommentUrge.Show.showSendMsgLogs();
...
//                  event.stopPropagation();
    ...
    CommentUrge.Show = CommentUrge.Show || {};
    CommentUrge.Show = $.extend({
...
//    listLog.Remarks = Remarks;
                    ...
                    callback: function(dataJson){
                        ...
                        if(!dataJson){
                            ...
                            if(!dataJson.isOk){
                                ...
                                var res=CommentUrge.Show.formatMsgLog(dataJson.res);
                                var html=$("#msg-log-tmpl").tmpl(res);
                                $(".msg-log-tbody").html(html);
                                CommentUrge.Init.initposhytip();
    ...
    CommentUrge.TradeSearch = CommentUrge.TradeSearch || {};
    CommentUrge.TradeSearch = $.extend({
        search : function(){
            var ruleData = CommentUrge.TradeSearch.getQueryRule();
                    ...
                    callback: function(dataJson){
                        if(!dataJson || !dataJson.isOk){
                            ...
                            if(dataJson.res != null && dataJson.res != undefined && dataJson.res.length > 0){
                                ...
                                $('.commentUrge-table').append(TM.CommentUrge.TradeSearch.createTbody(dataJson.res));

                                TM.CommentUrge.Event.setCopyLinkEvent();
                                TM.CommentUrge.Event.setRemarkSaveClickEvent();
                            } else {
        ...
        createTbody : function(results) {
            ...
            $(results).each(function(i,result){
                if(result.order.tidStr == 784706833508892) {
                    return;
                }
                ...
                //宝贝标题
                var goodTitle = TM.CommentUrge.TradeSearch.createTitle(result);
                var goodId = result.order.numIid;
                //订单号/收货时间
                var orderIdLink = '<a href="http://trade.taobao.com/trade/detail/trade_item_detail.htm?bizOrderId='+result.order.tidStr + '" target="_blank" style="font-size: 13px;*font-size: 12px;text-decoration: underline;">[订单号]'+result.order.tidStr + '</a>';
                ...
                //操作（复制评价链接）
                rate_link = 'https://rate.taobao.com/remarkSeller.jhtml?tradeID=' + result.order.tidStr;
                //备注
                var remark = '';
                ...
                tbody.append($('<tr buyerNick="' + buyerNick + '" receiverName="'+receiverName+'" phone="'+phone+'" tid="' + result.order.tidStr + '" oid="' + result.order.oidStr + '">' +
                    ...
                    '<td class="detail nick-info">' + receiverName + '<br>' + phone + '<br>' + buyerNick + TM.CommentUrge.Util.createBuyerWangWang(buyerNick) + '<br>[支付宝]' + buyerAlipayNo + '</td>' +
                    '<td><a class="copy_btn" data-clipboard-text="' + rate_link + '" style="cursor: pointer;">复制评价链接</a></td>' +
                    '<td><textarea rows="2" style="width:100%;height:100%;">' + remark + '</textarea></td>' +
                    '<td><button class="btn btn-primary save-remark" style="margin-left:3px;" tid="' + result.order.tidStr + '" oid="'+result.order.oidStr+'">保存</button></td></tr>'
                ));
                //checkbox的点击事件
        ...
        exportExcel: function (btn) {
            ...
            btn.attr('href', '/SkinComment/exportCommentUrgeExcel?' + param);
        }
    },CommentUrge.TradeSearch);

    CommentUrge.Event = CommentUrge.Event || {};
    CommentUrge.Event = $.extend({
        setStaticEvent : function(){
            TM.CommentUrge.Event.setTradeTypeClickEvent();
            TM.CommentUrge.Event.setTradeSearchBtnClickEvent();

            TM.CommentUrge.Event.setCheckAllClickEvent();

            TM.CommentUrge.Event.setNoteCareButtonEvent();
            TM.CommentUrge.Event.setBatchSelectedDispatchBtnClickEvent();
            TM.CommentUrge.Event.setRemarkSaveClickEvent();
            TM.CommentUrge.Event.setAddGroupClickEvent();
            TM.CommentUrge.Event.setDeleteGroupClickEvent();
            TM.CommentUrge.Event.bindServiceGroupEvent();
...
//            		TM.CommentUrge.TradeSearch.search();
            ...
            $('.export-btn').click(function(){
                TM.CommentUrge.TradeSearch.exportExcel($(this));
            });
        },
        ...
        setNoteCareButtonEvent : function () {
            ...
            function sendNoteSearched(templateId) {
                //获取请求数据
                var data = CommentUrge.TradeSearch.getQueryRule();
                //获取用户选择了哪个模板
                data.mid = templateId;
                //提示预估消耗短信数量
                var message = "";
        ...
        setBatchSelectedDispatchBtnClickEvent : function(){
            $('.dispatch-servicegroup-btn').click(function(){
                ...
                if($('.subCheck:checked').length == 0) {
                    ...
                    CommentUrge.ServiceGroup.dispatchGroupDialog();
                }
            });
        },
    ...
    CommentUrge.ServiceGroup = CommentUrge.ServiceGroup || {};
    CommentUrge.ServiceGroup = $.extend({
...
//                                TM.CommentUrge.Util.showRateResult(data);
    ...
    },CommentUrge.ServiceGroup);

    CommentUrge.Util = CommentUrge.Util || {};
    CommentUrge.Util = $.extend({
        setModelContent : function(){
            $.get('/autocomments/currContent',function(data){
                if(!data){
                    return;
        ...
        showRateResult : function(data){
            if(data.message == "content is empty"){
                ...
                $(data.res).each(function(i,failOrder){
                    var rate = TM.CommentUrge.Util.getRateResult(failOrder.result);
                    html += '<tr>' +
                        '<td>'+failOrder.tid+'</td>'+
                        '<td>'+failOrder.oid+'</td>'+
                        '<td>'+failOrder.buyerNick+'</td>'+
                        '<td>'+TM.CommentUrge.Util.createRateImg(rate)+'</td>'+
