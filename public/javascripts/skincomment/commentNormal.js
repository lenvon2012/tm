...
//            container.find(".date-picker").datepicker();
            ...
            TM.CommentNormal.Util.initRemarkSet();
                    ...
                    callback: function(dataJson){
                        if(!dataJson || !dataJson.isOk){
                            ...
                            if(dataJson.res != null && dataJson.res != undefined && dataJson.res.length > 0){
                            	...
                            	$('.commentNormal-table').append('<tbody class="skincomment-table-tbody" ><tr><td colspan="9" style="height:40px;">亲，暂无数据哦</td></tr></tbody>');
                            }
                        }
                    }
                }
...
//            $('.trade-type').find('span').eq(0).trigger("click");
        ...
        setTradeSearchBtnClickEvent : function() {
            ...
            $('.update-btn').click(function(){
            	$.get("/items/forceSyncTradeRates", function(data){
            		if(!data.success) {
