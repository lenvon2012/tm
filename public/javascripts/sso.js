...
function SSOController() {
    ...
    var updateCookieTimeHardLimit = 1800; // 鍦ㄨ鏃堕棿鍐呬笉鍏佽鏇存柊cookie锛岄粯璁�30鍒嗛挓
    var cookieExpireTimeLength = 3600 * 24;  // cookie杩囨湡鏃堕棿涓�24灏忔椂

    var crossDomainForward = null ;  // 骞挎挱鎿嶄綔瀹屾垚鍚庢墽琛岀殑鍔ㄤ綔
    var crossDomainTimer = null;
    var crossDomainTime = 3; // 鍏佽骞挎挱鎿嶄綔鐨勬渶澶ф椂闂�
    ...
    var loginMethod = ""; // post or get ,鏂逛究鐧诲綍鍚庣殑澶勭悊
    ...
    var loginByConfig = null;  // 涓哄啀娆＄櫥褰曚娇鐢�
    var loginMethodCheck = null;  // 涓哄啀娆＄櫥褰曚娇鐢�
    ...
    //闇€瑕侀厤缃殑鍐呭
    this.setDomain = false; // 涓哄悜涓嬪吋瀹规彁渚涚殑寮€鍚柊鐗规€х殑鍙傛暟
    this.feedBackUrl = '';
    this.service = 'sso';
    this.domain = 'sina.com.cn'; // sina.com.cn 鍩熺殑浜у搧涓嶈闇€瑕佽璁剧疆
    this.from = ''; // 璇锋眰鐨勫叿浣撴潵婧愶紝姣攅ntry鏇村叿浣�
    this.pageCharset = 'GB2312'; // 椤甸潰鐨勭紪鐮侊紝涓昏鏄敤浜庤璇�,榛樿涓篏B2312
    this.useTicket = false; // 鐧诲綍鎴愬姛鎴栧け璐ュ悗鏄惁杩斿洖ticket锛岄粯璁や负false
    this.isCheckLoginState = false; // 鍒濆鍖栨椂鏄惁妫€鏌ョ櫥褰曠姸鎬�, 妫€鏌ュ皢璋僨eedBackUrl ,鎵€浠ラ粯璁や笉妫€鏌�
    this.isUpdateCookieOnLoad = true; // 鏄惁鍦ㄥ姞杞借js鏂囦欢鍚庤嚜鍔ㄦ洿鏂颁竴娆ookie锛岄粯璁や负鑷姩鏇存柊,濡傛灉鑷畾涔変簡鍥炶皟鍑芥暟锛岃涓嶈鑷姩鏇存柊cookie
    this.useIframe = true; // 榛樿浣跨敤iframe 鍋歱ost鏂瑰紡鐨勭櫥褰曪紝濡傛灉涓嶈兘鎻愪緵feedBackUrl锛屽皢涓嶈兘浣跨敤璇ユ柟寮�,鏀归厤缃€夐」涓嶅嚭鐜板湪鏂囨。涓�
    this.noActiveTime = 7200; // 涓嶆椿璺冩椂闀匡紝榛樿2灏忔椂
    this.autoUpdateCookieTime = 1800; // 椤甸潰涓嶅姩鏃惰嚜鍔ㄦ洿鏂癱ookie鐨勯棿闅旀椂闂�,榛樿30鍒嗛挓,灏戜簬5s涓嶅仛鏇存柊
    this.loginType = rsa; // 榛樿rsa鏂瑰紡
    this.timeoutEnable = false;
    this.crossDomain = true; // default true
    this.scriptLoginHttps = false; // 褰撲娇鐢╬ost鏂瑰紡鐧诲綍澶辫触鍚庯紝杞崲涓簊cript鏍囩鏂瑰紡鐨勭櫥褰曟槸鍚﹀己鍒朵娇鐢╤ttps鐨勬柟寮忕櫥褰�,榛樿浣跨敤wsse鏂瑰紡鐧诲綍

    this.allowAutoFoundServerTime = false; //鏄惁鑷姩鍙戠幇鏈嶅姟鍣ㄧ鏃堕棿,榛樿涓嶈嚜鍔ㄥ彂鐜帮紝鑷冲皯闈炵櫥褰曢〉闈篃鍦ㄤ娇鐢ㄨjs锛岃繖浜涢〉闈㈠氨涓嶅叧蹇冩湇鍔″櫒鏃堕棿浜�
    this.allowAutoFoundServerTimeError = true; //鏄惁鑷姩鍙戠幇鏈嶅姟鍣ㄧ鏃堕棿閿欒
    ...
    this.failRedirect = false; //褰撻亣鍒伴渶瑕佽烦杞殑椤甸潰鏃讹紝褰撳墠椤甸潰鏄惁鏀寔璺宠浆銆�
    ...
    this.init = function() {
        me.setLoginType(me.loginType); // 妫€鏌ユ槸鍚﹂渶瑕佷娇鐢╤ttps
        ...
        //璁块棶feedBackUrl妫€鏌ョ櫥褰曠姸鎬侊紝feedBackUrl鍙兘浼氳浆鍚憇soLoginUrl鍙栫姸鎬�
        if(me.isCheckLoginState) {
            addEventListener(window,"load",function(){me.checkLoginState();});
        }
        //璇曞浘浠庨〉闈腑鍙戠幇ssoServerTime锛屼綔涓虹櫥褰曠殑鍒濆鏃堕棿
    ...
    /**
     * 姝ゅ嚱鏁颁负鐢ㄦ埛鍙鐩栫殑鍥炶皟鍑芥暟锛宭oginStatus鍙傛暟涓虹敤鎴风殑鐧诲綍鐘舵€�
     * 涓篺alse鍒欐湭鐧诲綍
     * 鍚﹀垯浼氭樉绀虹敤鎴风殑鐧诲綍淇℃伅鏁扮粍
     */
    ...
    this.login = function(username, password, savestate) {
        ...
        ssoLoginTimer.start(5000, function(){
            ssoLoginTimer.clear(); // 杩欓噷濡傛灉涓嶆竻闄ゆ椂閽燂紝鍒欒繜鍒扮殑鐧诲綍渚濈劧浼氭樉绀虹櫥褰曠粨鏋�
    ...
    this.updateCookie= function() {
        try {
            ...
            var result = {};
            if (cookieExpireTime == null) {  // cookie 涓嶅瓨鍦�
                result = {"retcode":6102}; // not login
            } else if( cookieExpireTime < now ) { // cookie 宸茬粡杩囨湡
                result = {"retcode":6203}; // cookie expired
            } else if( cookieExpireTime - cookieExpireTimeLength + updateCookieTimeHardLimit > now) { // 鍦ㄧ‖闄愬埗鐨勬椂闂村唴涓嶅厑璁告洿鏂癱ookie
                result = {"retcode":6110};
            } else if(cookieExpireTime - now > me.noActiveTime) {// 绂昏繃鏈熻繕鏃╃潃鍛�
                result = {"retcode":6111};
            }
    ...
    // 鐧诲綍鍥炶皟鍑芥暟,script鏍囩鐧诲綍鎴杝etDomain鏂瑰紡鐧诲綍鏃朵娇鐢ㄨ鍥炶皟鍑芥暟
    this.loginCallBack = function(result) {
        try {
            if(me.timeoutEnable && !ssoLoginTimer.isset()) return;
            ssoLoginTimer.clear();
            me.loginExtraFlag = {}; //娓呯┖涓庢湰娆＄櫥褰曠浉鍏崇殑鏍囪瘑
            var loginStatus = {};
            ...
            if(uid) {
                ...
                // 鏈嶅姟鍣ㄧ鏃堕棿璁剧疆涓嶆纭殑瀹归敊
                if (loginMethodCheck && result['retcode'] == "2092" && me.allowAutoFoundServerTimeError) {
                    me.setServerTime(0); // 閲嶇疆鏈嶅姟鍣ㄦ椂闂寸殑鍊硷紝渚夸簬閲嶆柊鑾峰彇
                    me.loginExtraFlag = objMerge(me.loginExtraFlag,{"wsseretry":"servertime_error"}); // 娣诲姞閲嶈瘯鏍囪瘑锛屼究浜庝簡瑙ｆ儏鍐�
                    loginMethodCheck(); // 閲嶆柊鐧诲綍涓€娆★紝杩欐娌℃湁浜嗙櫥褰曡秴鏃剁殑鎺у埗浜嗭紝瀹炵幇璧锋潵楹荤儲鑰屼笖娌℃湁蹇呰
                    loginMethodCheck = null; //閬垮厤姝诲惊鐜�
                    return false;
                }
                loginStatus['result'] = false;
                loginStatus['errno'] = result['retcode'];
                if (loginStatus['errno'] == '4069') { //灏濊瘯璺宠浆杩涜棰濆璁よ瘉
    ...
    this.updateCookieCallBack = function (result)  {
        ...
        } else { // 蹇界暐鏇存柊澶辫触鐨勬儏鍐碉紝鍥犱负鏇存柊澶辫触鍚庣殑鎿嶄綔涓嶆槸蹇呴』鐨�
            me.customUpdateCookieCallBack(result);
        }
    };
    this.feedBackUrlCallBack = function (result) {
        // 瀵逛簬缁欎簡feedBackUrl浣嗘槸鍙堝己鍒秅et鏂瑰紡鐧诲綍鐨勬椂鍊欙紝璁℃椂鍣ㄥ湪loginCallBack鍥炶皟鏃跺凡缁忔竻闄や簡
        if(loginMethod == "post" && me.timeoutEnable && !ssoLoginTimer.isset()) return;
        ssoLoginTimer.clear();
        if (result.errno == "2092") {
            me.setServerTime(0);	  // 閲嶇疆鏈嶅姟鍣ㄦ椂闂寸殑鍊硷紝渚夸簬閲嶆柊鑾峰彇
        }
        if (result.errno == '4069') { // 闇€瑕佽烦杞繘琛岄澶栬璇�
            var reason = result.reason.split('|');
        ...
        removeNode(me.loginFrameName);// 鍒犻櫎鐢ㄥ畬鐨刬frame,浣嗘槸涓€瀹氫笉瑕佸湪鍥炶皟鍓嶅垹闄わ紝Firefox鍙兘鏈夐棶棰樼殑
    };
    ...
    this.crossDomainCallBack = function(result) {
        // 濡傛灉瀛樺湪鑾峰彇骞挎挱鍦板潃鍒楄〃鐨勮姹傜殑script鏍囩锛屽垯杩欓噷宸茬粡鐢ㄨ繃锛屽垹闄や箣
        ...
        if(arrURL.length == 0) { // 鏈€鍒濅笉闇€瑕侀€氱煡浠讳綍鍩�
            clearTimeout(crossDomainTimer);
            me.crossDomainResult();
            return true;
        }
        for(var i = 0; i < arrURL.length; i++) {
            ...
            url = makeURL(url, request);
            if(isSafari()){
                //safari 闇€瑕佺敤杩欎釜鏂规硶	// http://wiki.intra.sina.com.cn/pages/viewpage.action?pageId=6297546
    ...
    this.crossDomainAction = function (action,callback) {
        ...
        if (crossDomainUrlList) { // 宸茬粡瀛樺湪浜嗛渶瑕佸箍鎾殑鍦板潃鍒楄〃
            me.crossDomainCallBack(crossDomainUrlList);
            return false;
        }
        // 杩樻病鏈夐渶瑕佸箍鎾殑鍦板潃鍒楄〃锛岃嚜宸遍€氳繃crossdomain.php 鏉ヨ幏鍙�
    ...
    this.setLoginType = function(loginType) {
        // 妫€鏌ュ綋鍓嶉〉闈㈡槸鍚ttps锛屽鏋滄槸锛屽垯寮哄埗浣跨敤https鐧诲綍锛屽惁鍒欐祻瑙堝櫒鏈夋彁绀�
    ...
    var makeRequest = function(username, password, savestate) {
        ...
        request["su"] = sinaSSOEncoder.base64.encode(urlencode(username)); // su鏄痷sername鐨勫埆鍚�
        ...
        if((me.loginType & rsa) && me.servertime && sinaSSOEncoder && sinaSSOEncoder.RSAKey){
            ...
            password = sinaSSOEncoder.hex_sha1("" + sinaSSOEncoder.hex_sha1(sinaSSOEncoder.hex_sha1(password)) + me.servertime + me.nonce); // 绌哄瓧绗︿覆涓轰簡閬垮厤鍚勯」鍏ㄩ儴鏄暟瀛楁椂閫犳垚鏁板瓧鐩稿姞鑰屼笉鏄瓧绗︿覆閾炬帴鐨勬儏鍐�
        }
        request["sp"] = password; // sp鏄痯assword鐨勫埆鍚�
        return request;
    };
    // login by script
