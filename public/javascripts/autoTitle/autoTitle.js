                        ...
                        $.get('/status/user',function(data){
                            ...
                            if(interval > 7*24*3600*1000){
                                var html = ''+
                                    ...
                                    '<a style="margin-left: 110px;" target="_blank" href="http://www.taobao.com/webww/ww.php?ver=3&touid=%E9%AA%91%E7%9D%80%E7%BB%B5%E7%BE%8A%E9%A3%9E%3A%E5%B0%8F%E9%9B%AA&siteid=cntaobao&status=1&charset=utf-8"><img border="0" src="http://img04.taobaocdn.com/tps/i4/T1uUG.XjtkXXcb2gzo-77-19.gif" alt="联系售后"></a>' +
                                    '</td>'+
                                    '</tr>' +
                                    '</tbody>'+
                                    '</table>'+
...
//        arr.push('<li><a targetcls="competition" href="javascript:void(0);">类目优秀标题</a></li>');
       ...
       /* arr.push('<li><a targetcls="catAnalysis" href="javascript:void(0);">行业分析</a></li>');*/
        ...
        tabHolder.find('a').click(function(){
            ...
            switch(tabClicked.attr('targetcls')){
                ...
                case '200words':
                    ...
                    break;
               /* case 'catAnalysis':
                    me.initCatAnalysis(contentHolder,numIid);
                    break;*/
                /*case 'CWords':
    ...
    me.gen200words = function(container, numIid, orderBy, isDesc){
        ...
        $.ajax({
            on:true,
            dataType: 'jsonp',
            url: 'http://chedao.taovgo.com/commons/tmAllRecommend?numIid='+numIid + '&orderBy=' + orderBy + '&isDesc=' + isDesc,
            success:function(data){
    ...
    me.initCatAnalysis = function(container, numIid) {
        container.empty();
        var selects = '<div class="catgory">' +
            ...
            '</div>';
        container.append($(selects));
        me.initLevelSelect(container);
        /*var htmls = [];
...
//            htmls.push('<table><thead><th>属性名</th><th>热销属性词</th></thead><tbody>');
    ...
    me.initLevelSelect = function(container){
        $.ajax({
            //async : false,
            ...
            success : function(data) {
                ...
                me.Level2Select(container);
            }
        });
    }

    me.Level2Select = function(container){
        $(".selectCat1").unbind();
    ...
    me.CreateTable = function(container){
        ...
        $('.batchSelectUseRecommend').click(function(){
            ...
            $.each($(".itemsWithDiagResult").find(".subCheckBox:checked"),function(i, input){
               ...
               /* idArr.push(oThis.attr('numIid'));
                ...
                numIidToTitleArr.push(oThis.attr('numIid') + "!@#" + oThis.parent().parent().parent().find(".newTitleContent").val());
            });
            //timeout: 30000s
            // numIids:idArr.join(','),titles:titles.join(',:,')

            $.ajax({
                url : '/titles/newBatchChange',
                data:{numIidToTitle : numIidToTitleArr.join("#@!")},
                timeout: 200000,
                type:'post',
                success : function(data){
