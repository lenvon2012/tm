 ...
 *    actionURL:'',     //URL to do with the selected items
 *    pn:1,
 *    ps:8,
...
multiOpByURL.createChoose= $.extend({
    createOrRefleshCommsDiv:function(options){
        ...
        //alert(options.itemsURL);alert(options.actionURL);alert(options.ps);alert(options.pn);alert(options.enableSearch);
    ...
    createItemChooseSearch:function(options){
        ...
        itemChooseSearch.append($('<select id="multiSearch-sellerCat" style="float: left;margin: 15px;width: 140px;display: none;"></select>'));
        itemChooseSearch.append($('<select id="multiSearch-taobaoCat" style="float: left;margin: 15px;width: 140px;display: none;"></select>'));

        $.get("/items/sellerCatCount",function(data){
            var sellerCat = itemChooseSearch.find('#multiSearch-sellerCat');
            sellerCat.empty();
        ...
        $.get("/items/itemCatCount",function(data){
            var taobaoCat = itemChooseSearch.find('#multiSearch-taobaoCat');
            taobaoCat.empty();
            if(!data || data.length == 0){
                taobaoCat.remove();
            }
...
multiOpByURL.fillComms= $.extend({
    getCommsList:function(options){
        ...
        data.pn=options.pn;
        data.ps=options.ps;
        var sellerCat = $('#multiSearch-sellerCat option:selected').attr("catid");
        if(sellerCat === undefined || sellerCat == null) {
            data.sellerCid = "";
        } else {
            data.sellerCid = sellerCat;
        }
        var taobaoCat = $('#multiSearch-taobaoCat option:selected').attr("catid");
        if(taobaoCat === undefined || taobaoCat == null) {
            data.cid = "";
        } else {
            data.cid = taobaoCat;
        }
...
multiOpByURL.Event= $.extend({
    ...
    setCommImgEvent:function(options){
        $(".commImg").click(function(){
            ...
            if(commMask.html()=="点击图片添加"){
             ...
             //   multiOpByURL.rule.numIidList = multiOpByURL.rule.numIidList+commMask.parent().attr("numIid")+",";
                options.numIidList = options.numIidList+commMask.parent().attr("numIid")+",";
            }
            else if(commMask.html()=="已选中"){
                commMask.html("点击图片添加");
                commMask.css("background-color","#aaa");
                commMask.parent().css("border-color","#aaa");
               // multiOpByURL.rule.numIidList=multiOpByURL.rule.numIidList.replace((commMask.parent().attr("numIid")+","),"");
                options.numIidList=options.numIidList.replace((commMask.parent().attr("numIid")+","),"");
            }
    ...
    setSelectOKEvent:function(options){
        ...
        $("#selectOK").click(function(){
            var params = {};
            if(options.relation.isRelation){
                params.numIids=options.numIidList;
                params.planId = options.relation.relationPlanId;
                params.modelId = options.relation.modelId;
                params.newName = options.relation.newName;
            }
            if("/Relation/removeRelatedItems"==options.actionURL)  {
                params.planId = options.relation.relationPlanId;
                params.numIids = options.numIidList;
            }
            else {
            ...
            if(options.waterMark.waterCallback != ""){
                options.waterMark.waterCallback(options.numIidList);
            }
            $.ajax({
...
//                    TM.Alert.load("选择成功");
                    ...
                    if (options.actionCallback === undefined || options.actionCallback == null) {

                    } else {
                        options.actionCallback(data);
