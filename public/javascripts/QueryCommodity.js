...
QueryCommodity.query = $.extend({
    getComms:function(currentPage){
        $('#sellerCat').find('option').eq(0).attr("selected",true);
       ...
       /* $.ajax({
        ...
        //var top = $('<div class="commodityDiagBottom" style="text-align: center;"></div>');
        //var bottom = $('<div class="commodityDiagBottom" style="text-align: center;"></div>');
        ...
        $('.tmpage').tmpage({
            ...
            ajax: {
                ...
                callback:function(data){
                    ...
                    console.info(diagContainer.find('.question'));
                    diagContainer.find('.question').qtip({
                        content: {
                            text: "标题达到85分以上就算不错了哟"
                        },
                        position: {
                            at: "top left ",
                            corner: {
...
QueryCommodity.commodityDiv = $.extend({
	 ...
	 createDiv: function(DiagResult, pageData,data, offset) {
         var  res= $('<div class="singleComm"></div>');
         var titleDiv=QueryCommodity.commodityDiv.createTitleDiv(DiagResult,pageData, data, offset);
         var detail=QueryCommodity.commodityDiv.createDetail(DiagResult);
         res.append(titleDiv);
         res.append(detail);
...
//        upper.append(imgObj2);
		var baseInf=$('<div id="baseInf"></div>')
        ...
        var lower=$('<div style="height:24px;font-size:16px;"></div>');
	...
	/*	var commImg=$('<div class="commodityImg"></div>');
        ...
        */
        ...
        var imgObj=$('<img class="commImg" />');
        imgObj.attr("src",DiagResult.picPath);
		var url = "http://item.taobao.com/item.htm?id=" + DiagResult.numIid;
		aObj.append(imgObj);
		aObj.attr("href",url);
        return aObj;
     ...
     createBtn:function(DiagResult, pageData,data, offset){
		...
		var btnObj=$('<div class="youhuaBtn"></div>');
...
//                 href="javascript:void(0)";
	...
	createDetail: function(DiagResult) {
		 var diagDetail=$('<div class="diagDetail"></div>');
