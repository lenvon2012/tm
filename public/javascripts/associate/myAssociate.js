    ...
    myAssociate.setEvent = $.extend({
        ...
        toPut:function(template){
            template.find(".associateToPut").click(function(){
                 var name = template.find(".tzg_tag_name").attr('name');
                 var planId = name.substring(12,name.length);
                 window.location.href = "/associate/associate?id=" + planId + "&toput=true";
            });
        },
        toModify:function(template){
            template.find(".associateModify").click(function(){
                var name = template.find(".tzg_tag_name").attr('name');
                var planId = name.substring(12,name.length);
                window.location.href = "/associate/associate?id=" + planId;
                //IE
        ...
        toModifyInPut:function(template){
            template.find(".associateModify").click(function(){
                ...
                var name = template.find(".tzg_tag_name").attr('name');
                var planId = name.substring(12,name.length);
                window.location.href = "/associate/associate?id=" + planId + "&input=true";
                //IE
                window.event.returnValue = false;
            });
        },
            ...
            $(".toAddItemBtn").unbind("click").click(function(){
                ...
                if(!itemIidsArr.length > 0){
                   ...
                   return false;
                    ...
                    TM.Alert.showDialog('<a>确定要投放以下<span class="successNum">' + itemIidsArr.length + '</span>个宝贝？</a>', 200,150, function(){TM.myAssociate.listItem.toPut(itemIids,planId,modelId)},function(){return false},"提示");
                }
            });
        },
