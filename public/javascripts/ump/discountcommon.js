...
((function ($, window) {
    ...
    DiscountCommon.init = $.extend({
        ...
        doInit: function(container, isCreateActivity, nowTime) {
            ...
            //设置event
            DiscountCommon.event.initTimeEvent(nowTime);
...
//            DiscountCommon.checker.checkActivityTitle();
    ...
    DiscountCommon.event = $.extend({
        initTimeEvent: function(nowTime) {
            ...
            var minStartDate = new Date(nowTime);
            var startTimeStr = startTimeObj.val();
            if (startTimeStr === undefined || startTimeStr == null || startTimeStr == "") {

            } else {
                var tempStartDate = new Date(startTimeStr);
                if (tempStartDate.getTime() < nowTime) {
            ...
            initEndTimePicker(new Date(nowTime));
        ...
        initNormalEvent: function() {
            ...
            $.ajax({
                ...
                success : function(dataJson) {
                	...
                	var jsonArray = dataJson.res;
						
					$(jsonArray).each(function(index, json) {
						if(json.clazz == "促销") {
							$(".target-activity-title").append($('<option value='+json.words+'>' + json.words + '</option>'));
						}
					});
					
					$(".activity-title-clazz-select").unbind("change").change(function() {
						var clazz = $(this).val();
						$(".target-activity-title").empty();
						$(jsonArray).each(function(index, json) {
							if(json.clazz == clazz) {
								$(".target-activity-title").append($('<option value='+json.words+'>' + json.words + '</option>'));
							}
