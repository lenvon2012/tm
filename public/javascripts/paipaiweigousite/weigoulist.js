...
((function ($, window) {
    ...
    WeigouList.init = $.extend({
        doInit: function(container) {
            ...
            var html = WeigouList.init.createHtml();
            container.html(html);
...
//            $(".masonry-div").imagesLoaded(function() {
//            	$('.masonry-div').masonry('reload');
//            });
    ...
    WeigouList.show = $.extend({
        ...
        firstShow: function() {
            ...
            WeigouList.show.doShow(data, '/PaiPaiPromoteSite/randomUserItems');
        },
        scrollShow: function() {
            var data = {};
            WeigouList.show.doShow(data, '/PaiPaiPromoteSite/randomItems');
        },
...
//                    });
    ...
    WeigouList.item = $.extend({
        doInit: function(itemJson) {
            ...
            var href = "/PaiPaiPromoteSite/detail?numIid=" + itemJson.numIid;
            var topCatId = TM.WeigouBase.util.getSearchCat();
            if (topCatId === undefined || topCatId == null) {
            ...
            itemObj.find(".item-img").attr("src", itemJson.picPath.replace(".jpg", ".200x200.jpg"));
            itemObj.find(".item-img").css("height", "190px");
            ...
            if (skuMinPrice === undefined || skuMinPrice == null || skuMinPrice <= 0 || skuMinPrice >= originPrice) {
                ...
                itemObj.find(".item-origin-price").html(itemJson.price/100);
                if(itemJson.skuMinPrice > 0){
                    itemObj.find(".item-new-price").html(itemJson.skuMinPrice/100);
                } else {
                    itemObj.find(".item-new-price").html(itemJson.price/100);
                }
        ...
        createHtml: function() {
            var html = '' +
                ...
                '           <img style="width: 190px;" class="item-img" />' +
                '       </a>' +
                '   </div> ' +
