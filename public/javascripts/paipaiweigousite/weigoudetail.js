...
((function ($, window) {
    ...
    WeigouDetail.show = $.extend({
        doShow: function() {
            ...
            $.ajax({
                url : "/paipaipromotesite/queryPopularized",
                data : data,
                type : 'post',
    ...
    WeigouDetail.item = $.extend({
        createItemObj: function(itemJson) {
            ...
            var itemObj = $(html);
            var href = "http://item.wanggou.com/" + itemJson.itemCode;

            itemObj.find(".item-href").attr("href", href);
            itemObj.find(".item-href").attr("target", "_blank");
            itemObj.find(".item-img").attr("src", itemJson.picPath.replace(".jpg",".400x400.jpg"));
        ...
        createHtml: function() {
            var html = '' +
                ...
                '   <tbody>' +
                '   <tr>' +
                '       <td><a class="item-href" target="_blank" style="display: block;"><img class="item-img" style="width: 480px;" /></a></td> ' +
