...
((function ($, window) {
    ...
    WeigouBase.init = $.extend({
        ...
        setSelectedNav: function() {
            TM.gcs('/js/jquery.url.js', function () {
                ...
                if (topCatId === undefined || topCatId == null || topCatId <= 0) {
                    navObj.find(".first-page-link").addClass('selected-nav');
                } else {
                    navObj.find('a').each(function(){
                        var anchor = $(this);
                        var tempCatId = WeigouBase.util.getSearchCatFromHref(anchor.attr('href'));
                        if(tempCatId == topCatId){
    ...
    WeigouBase.event = $.extend({
        doInit: function() {
            ...
            $(".item-search-btn").click(function() {
                ...
                var catId = WeigouBase.util.getSearchCat();
                if (catId === undefined || catId == null || catId == "" || catId <= 0) {
                    catId = 0;
                }
                var title = $(".item-search-input").val();
                if ($(".item-search-input").hasClass("item-search-tip-color")) {
                ...
                var action = "";
                if (title == "") {
                    action = '/taoweigou/cat' + catId + '.html';
                } else {
                    action = '/cat' + catId + "/s/" + title + ".html";
                }
    ...
    WeigouBase.util = $.extend({
        ...
        getSearchCat: function() {
            ...
            return WeigouBase.util.getSearchCatFromHref(href);
        },
        getSearchCatFromHref: function(href) {
            var params = $.url(href);
            var topCatId = params.param('catid') || params.param('catId');
            if (topCatId === undefined || topCatId == null || topCatId == "") {
                var startIndex = href.indexOf("/cat");
                if (startIndex > 0) {
                    startIndex += "/cat".length;
                    var endIndex = href.indexOf("/s/", startIndex);
                    if (endIndex < 0) {
                        endIndex = href.indexOf(".html", startIndex);
                    }
                    if (endIndex > 0) {
                        var idStr = href.substring(startIndex, endIndex);

                        topCatId = parseInt(idStr);
                    }
        ...
        getItemSearchParam: function() {
            ...
            //是/s/search.html格式的
            if (searchTitle === undefined || searchTitle == null || searchTitle == "") {
                var startIndex = href.indexOf("/s/");
                if (startIndex > 0) {
                    startIndex += "/s/".length;
                    var endIndex = href.indexOf(".html", startIndex);

                    if (endIndex > 0) {
                        searchTitle = href.substring(startIndex, endIndex);
...
((function ($, window) {
    ...
    WeigouHot.show = $.extend({
        ...
        firstShow: function() {
            ...
            WeigouHot.show.doShow(data, '/shopping/randomUserItems');
        },
        scrollShow: function() {
            var data = {};
            WeigouHot.show.doShow(data, '/shopping/randomItems');
        },
        doShow: function(data, url) {
            ...
            $.ajax({
                dataType: 'jsonp',
                url: 'http://121.199.28.225/shopping/innerRandomItems?topCatId=' + data.topCatId + '&title=' + data.title + '&status=2',
    ...
    WeigouHot.item = $.extend({
        doInit: function(index, itemJson) {
            ...
            if (skuMinPrice === undefined || skuMinPrice == null || skuMinPrice <= 0 || skuMinPrice >= originPrice) {
                ...
                itemObj.find(".origin-item-price").html(itemJson.price);
                itemObj.find(".item-price").html(itemJson.skuMinPrice);
            }
