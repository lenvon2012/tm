...
//                '               <td><span class="tmbtn short-green-btn batch-delist-btn">选中下架</span> </td>' +
                '               <td><span class="btn btn-success batch-modify-price-btn">修改选中宝贝价格</span> </td>' +
                '               <td><span class="btn btn-success do-update-price-btn">修改所有宝贝价格</span> </td>' +
                '               <td><span class="btn btn-info update-fenxiao-price-btn">同步分销商最低零售价</span> </td>' +
                '               <td><span class="btn btn-info update-fenxiao-price-btn">同步分销商最高零售价</span> </td>' +
            ...
            SkinBatch.container.find(".update-fenxiao-price-btn").click(function() {
                TM.Loading.init.hidden();
                SkinBatch.init.initUpdateAllFenxiaoDialog();
            });

            SkinBatch.container.find(".update-fenxiao-low-price-btn").click(function() {
                TM.Loading.init.hidden();
                SkinBatch.init.initUpdateFenxiaoLowDialog();
            });

            SkinBatch.container.find(".update-fenxiao-high-price-btn").click(function() {
                TM.Loading.init.hidden();
                SkinBatch.init.initUpdateFenxiaoHighDialog();
            });

            SkinBatch.container.find(".do-update-price-btn").click(function() {
                TM.Loading.init.hidden();
                SkinBatch.init.initDoUpdateAllPriceDialog();
            });
        },
        //宝贝表格
        ...
        initDoingDialog: function() {
            var dialogObj = $(".dialog-div");
            dialogObj.empty();
            var html = '' +
                '<div class="dialog-div" style="background:#fff;">' +
                '   <div class="loading-div" style="text-align: center;padding: 20px;font-weight: bold;color: red;"><img src="/public/images/fenxiao/loading.gif" /><span class="loading-text">正在处理第0个到第10个宝贝</span></div>' +
                '</div> ' +
                '';
...
//                    },
        ...
        initUpdateAllFenxiaoDialog: function() {
            var dialogObj = SkinBatch.init.initDoingDialog();

            TM.Loading.init.hidden();

            SkinBatch.submit.doUpdateAllPrice(dialogObj, 1);
        },
        initUpdateFenxiaoLowDialog: function() {
            var dialogObj = SkinBatch.init.initDoingDialog();

            TM.Loading.init.hidden();

            SkinBatch.submit.doUpdateAllLowPrice(dialogObj);
        },
        initUpdateFenxiaoHighDialog: function() {
            var dialogObj = SkinBatch.init.initDoingDialog();

            TM.Loading.init.hidden();

            SkinBatch.submit.doUpdateAllHighPrice(dialogObj);
        },

        initDoUpdateAllPriceDialog: function() {
            var dialogObj = SkinBatch.init.initDoingDialog();
    ...
    SkinBatch.row = $.extend({
        createRow: function(index, itemJson) {
            var html = '' +
                ...
                '   </td>' +
                '   <td>' +
                '       <div class="tmbtn sky-blue-btn item-modify-price-btn">修改价格</div>' +
...
//                '       <div class="tmbtn sky-blue-btn item-delist-btn"></div>' +
    ...
    SkinBatch.submit = $.extend({
        ...
        updateFenxiaoPrice: function(dialogObj, pn){
            TM.Loading.init.hidden();
            $.get("/fenxiao/updateAllFenxiaoPrice", {pn: pn, ps:10}, function(data){
                if(data && data.count == 10) {
                    dialogObj.find(".loading-text").html("正在处理第" + data.pn * 10 + "到" + (data.pn+1)*10 +"个宝贝");
                    SkinBatch.submit.updateFenxiaoPrice(dialogObj, ++pn);
                }
            });
        },

        doUpdateAllPrice: function(dialogObj, pn){
            $.ajax({
                url : '/skinbatch/checkAuth',
                data : {},
                type : 'post',
                success : function(dataJson) {
                    ...
                    TM.Loading.init.hidden();
                    $.get("/fenxiao/doUpdateAllPrice", {pn: pn, ps:10}, function(data){
                        if(data && data.count == 10) {
                            dialogObj.find(".loading-text").html("正在处理第" + data.pn * 10 + "到" + (data.pn+1)*10 +"个宝贝");
                            SkinBatch.submit.updateFenxiaoPrice(dialogObj, ++pn);
                        }
        ...
        updateFenxiaoLowPrice: function(dialogObj, pn){
            TM.Loading.init.hidden();
            $.get("/fenxiao/updateFenxiaoLowPrice", {pn: pn, ps:10}, function(data){
                if(data && data.count == 10) {
                    dialogObj.find(".loading-text").html("正在处理第" + data.pn * 10 + "到" + (data.pn+1)*10 +"个宝贝");
                    SkinBatch.submit.updateFenxiaoLowPrice(dialogObj, ++pn);
                }
            });
        },
        doUpdateAllLowPrice: function(dialogObj){
            $.ajax({
                url : '/skinbatch/checkAuth',
                data : {},
                type : 'post',
                success : function(dataJson) {
                    ...
                    TM.Loading.init.hidden();
                    SkinBatch.submit.updateFenxiaoLowPrice(dialogObj, 1);

                }
            });
        },

        updateFenxiaoHighPrice: function(dialogObj, pn){
            TM.Loading.init.hidden();
            $.get("/fenxiao/updateFenxiaoHighPrice", {pn: pn, ps:10}, function(data){
                if(data && data.count == 10) {
                    dialogObj.find(".loading-text").html("正在处理第" + data.pn * 10 + "到" + (data.pn+1)*10 +"个宝贝");
                    SkinBatch.submit.updateFenxiaoHighPrice(dialogObj, ++pn);
                }
            });
        },
        doUpdateAllHighPrice: function(dialogObj){
            $.ajax({
                url : '/skinbatch/checkAuth',
                data : {},
                type : 'post',
                success : function(dataJson) {
                    ...
                    TM.Loading.init.hidden();
                    SkinBatch.submit.updateFenxiaoPrice(dialogObj, 1);
                }
            });
        }
    }, SkinBatch.submit);
