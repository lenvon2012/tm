...
((function ($, window) {
    ...
    TM.appShopSearchWords = TM.appShopSearchWords || {};

    var appShopSearchWords = TM.appShopSearchWords;

    appShopSearchWords.init = appShopSearchWords.init || {};
    appShopSearchWords.init = $.extend({
        doInit : function(container) {
            appShopSearchWords.init.Container = container;
            var interval = appShopSearchWords.util.getDaysBetween();
			if(interval < 1) {
				$('.searchDiagTip').show();
				return;
			} else if (interval < 3 ) {
            ...
            // 设置相关事件
            appShopSearchWords.Event.setEvent(container);
        }
    }, appShopSearchWords.init);

    appShopSearchWords.Diag = appShopSearchWords.Diag || {};
    appShopSearchWords.Diag = $.extend({
        shopWordsDiag : function(targetDiv, interval, endTime, orderBy, isDesc) {
            ...
            var table = appShopSearchWords.init.Container.find('.word-diag-result-table');
                    ...
                    callback: function(data){
                        ...
                        if(data.res.length > 0) {
                            ...
                            table.find('tbody .pvTrend').unbind('click').click(function(){
                                ...
                                $.post("/Diag/appShopWordTrend", {interval:interval, endTime:endTime, word:word}, function(data){
                                    if(data === undefined || data == null) {
                                        TM.Alert.load("查不到该关键词数据，请重试或联系客服");
                                    }
                                    appShopSearchWords.util.drawChart(data, interval, word);
                                });
                            });
                        } else {
        ...
        createItemDiagTable : function(){
            var table = appShopSearchWords.Diag.createTableHtml();
    ...
    appShopSearchWords.Event = appShopSearchWords.Event || {};
    appShopSearchWords.Event = $.extend({
        setEvent : function(container) {
            appShopSearchWords.Event.setIntervalEvent2();
            appShopSearchWords.Event.setOrderByEvent();
            appShopSearchWords.Event.setExportExcel2();
        },
        setOrderByEvent : function(){
            appShopSearchWords.init.Container.find('.word-diag-result-table-th .orderTd').click(function(){
                var interval = appShopSearchWords.util.getInterval2();
                var endTime = appShopSearchWords.util.getEndTime2();
                var orderBy = $(this).find('.sort').attr("sort");
                var isDesc = $(this).hasClass('desc');
                isDesc ? $(this).removeClass('desc') : $(this).addClass('desc');
                appShopSearchWords.Diag.shopWordsDiag(appShopSearchWords.init.Container.find('.diag-result-div'),
        ...
        setIntervalEvent : function(){
            ...
            startTimeInput.unbind('change').change(function(){
                appShopSearchWords.init.Container.find('.interval-tr .interval[value="0"]').trigger("click");
            });
            endTimeInput.unbind('change').change(function(){
                appShopSearchWords.init.Container.find('.interval-tr .interval[value="0"]').trigger("click");
            });
            appShopSearchWords.init.Container.find('.interval-tr .interval').unbind('click').click(function(){
            ...
            appShopSearchWords.init.Container.find('.interval-tr .interval:checked').trigger('click');
        },
        ...
        // 导出按钮点击事件
        setExportExcel: function(){
            $("[name='export_excel']").click(function(){
                var interval = $('.interval-tr').find('input:radio:checked').val();
                var curr = new Date().getTime();
                var dayMillis = 24 * 3600 * 1000;
                ...
                form.attr('action', "/Diag/exportExcelAll");
    ...
    }, appShopSearchWords.Event);

    appShopSearchWords.util = appShopSearchWords.util || {};
    appShopSearchWords.util = $.extend({
        ...
        getPlatform : function(){
            var target = appShopSearchWords.init.Container.find('.shop-pc-online-info .opTabWrapper .selected').attr("target");
            switch (target){
                case "shop" :
                    return 0;
                case "pc" :
        ...
        drawChart : function(pvMap, interval, word){
            var dayArr =  appShopSearchWords.util.genDayArr(interval);
            var pvArray = appShopSearchWords.util.genPvArr(interval, pvMap);
